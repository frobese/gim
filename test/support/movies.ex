defmodule GimTest.Movies do
  ...
  def demo_import() do
    ...
    IO.puts(
      "Time: #{Float.round(secs, 1)} s, Memory usage: #{Float.round(memory, 1)} MB, #{
        Float.round(ops, 1)
      } us/op"
    )
  ...
  def resolve_node(%{__struct__: Genre} = node, map) do
    %{node | movies: node.movies |> resolve_edges(map)}
  end

  def resolve_node(%{__struct__: Character} = node, map) do
    %{node | performances: node.performances |> resolve_edges(map)}
  end
